...
def main():
    ...
    def TitleWriting(game, newname = '.', status = 'pass'):
        ...
        if newname == '.':
            ...
            TitleWriter.writerow([game, BGGraw_names[ID], int(PAXids[PAXnames.index(game)]), ID, lock])
        elif newname == '':
            TitleWriter.writerow([game, game, int(PAXids[PAXnames.index(game)]), 0, True])
            ErrorWriter.write(game + '\n')
        elif status == 'fail':
            TitleWriter.writerow([newname, newname, int(PAXids[PAXnames.index(game)]), 0, True])
            ErrorWriter.write(newname + '\n')
        else:
            ID, lock = DupesCheck(newname)
            TitleWriter.writerow([newname, BGGraw_names[ID], int(PAXids[PAXnames.index(game)]), ID, lock])
        return()
    ...
    header = ['Title', 'Raw Title', 'PAX ID', 'BGG ID', 'Status']
    ...
    try:
        BGGgames = open('BGG_IDs.csv', 'r', encoding='utf-16')
    except: 
    ...
    # Second dictionary (BGGraw_names) has BGG ID#s as keys and the fully-accented title strings as values
    ...
    for rows in BGGreader:
        if str(rows[1]) in BGGnames.keys():
            BGGnames[rows[1]].update({rows[0] : rows[2]})
            BGGraw_names[rows[0]] = rows[4]
        else:
            BGGnames[rows[1]] = {rows[0] : rows[2]}
            BGGraw_names[rows[0]] = rows[4]
